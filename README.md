...
|---|----------|----------|----------|--------|-----------|
| 1 | Document | How many casual leaves is an employee entitled to per year? | 12 casual leaves per year with source citation | Employees are entitled to 12 casual leaves per calendar year. [Source: leave_policy.txt, Section 2.1] Confidence: High (91%) | ✅ Pass |
| 2 | Document | If a procurement request above Rs 1,00,000 is submitted, who needs to approve it? | CFO and MD jointly, with timeline | Approval granted jointly by CFO and MD. Process completed within 10 working days. [Source: procurement_sop.txt, Section 4.4] Confidence: High (90%) | ✅ Pass |
| 3 | Document | What happens if a shortlisted candidate doesn't respond to an interview invitation? | Reminder sent, then marked non-responsive, next candidate contacted | Follow-up reminder sent within 48 hours. After 72 hours total: marked non-responsive, next waitlist candidate contacted. [Source: candidate_screening_sop.txt, Section 6.2] Confidence: High (93%) | ✅ Pass |
| 4 | Document | Can an employee take earned leave during probation? | No — earned leave cannot be availed during probation | Employees under probation (first 6 months) may not avail earned leave. [Source: leave_policy.txt, Section 3.4] Confidence: High (91%) | ✅ Pass |
| 5 | Structured Data | Which branch has the highest placement rate? | Correct branch computed from CSV | Bangalore branch has the highest placement rate. (computed via Pandas agent) | ✅ Pass |
| 6 | Structured Data | What is the average package across all branches? | Computed mean from CSV | The average package across all branches is 6.53 LPA. | ✅ Pass |
| 7 | Tool Calling | What is the current weather in Mumbai? | Live data with temperature, humidity, wind | Few clouds, 28.87°C (feels like 33.38°C), humidity 75%, wind 2.08 m/s, visibility 10.0 km. [Live data from OpenWeatherMap API] | ✅ Pass |
| 8 | Refusal | What is the CEO's annual bonus as per company policy? | Hard refusal — topic not in any document | I could not find sufficient information in the available documents to answer this. | ✅ Pass |
